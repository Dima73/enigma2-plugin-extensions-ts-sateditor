...
class Transponder:
    ...
    transSystem = {"0": "DVB-S", "1": "DVB-S2", "dvb-s": "DVB-S", "dvb-s2": "DVB-S2"}
    ...
    reTransPolarisation = {"H": "0", "V": "1", "L": "2", "R": "3"}
    transModulation = {
        "0": "AUTO",
        "1": "QPSK",
        "2": "8PSK",
    ...
    transRolloff = {"0": "0_35", "1": "0_25", "2": "0_20", "3": "Auto"}
    reTransRolloff = {"0_35": "0", "0_25": "1", "0_20": "2", "Auto": "3"}
    ...
    onlyDVBS2Fec = ["FEC_8_9", "FEC_3_5", "FEC_4_5", "FEC_9_10"]
    transBand = {"KU": ("10700000", "12750000"), "C": ("3400000", "4200000")}
    transPlsMode = {"0": "Root", "1": "Gold", "2": "Combo", "3": "Unknown"}
    reTransPlsMode = {"Root": "0", "Gold": "1", "Combo": "2", "Unknown": "3"}
    ...
    def transponderDoctor(self, transponder):
        ...
        if len(missing):
            ...
            return
        self.polarisation = self.transPolarisation.get(
            transponder.get(transParam.get("polarization"), "i").lower()
        )
        if self.polarisation == "i":
            print(
                "transponderDoctor: unbekannter Wert fuer Polarisation (%s)"
                % transParam.get("polarization")
            )
            return
        ...
        if "system" in transParam:
            self.system = self.transSystem.get(
                transponder.get(transParam.get("system"), "i").lower()
            )
        ...
        if "modulation" in transParam:
            self.modulation = self.transModulation.get(
                transponder.get(transParam.get("modulation"), "i").lower()
            )
            if self.modulation == "8PSK" or self.modulation == "QAM16":
                dvb_s2_cnt += 1
        if "pilot" in transParam:
            self.pilot = self.transOnOff.get(
                transponder.get(transParam.get("pilot"), "i").lower()
            )
            if self.pilot == "ON" or self.pilot == "AUTO":
                dvb_s2_cnt += 1
        if "rolloff" in transParam:
            self.rolloff = self.transRolloff.get(
                transponder.get(transParam.get("rolloff"), "i").lower()
            )
            if self.rolloff == "0_25":
                dvb_s2_cnt += 1
        if "fec_inner" in transParam:
            self.fec = self.transFec.get(
                transponder.get(transParam.get("fec_inner"), "i").lower()
            )
        ...
        if "inversion" in transParam:
            self.inversion = self.transOnOff.get(
                transponder.get(transParam.get("inversion"), "i").lower()
            )
        ...
        if "pls_mode" in transParam:
            self.plsmode = self.transPlsMode.get(
                transponder.get(transParam.get("pls_mode"), "i").lower()
            )
            self.UseMultistream = True
        ...
        if "t2mi_plp_id" in transParam:
            self.__t2mi_plp_id = transponder.get(
                transParam.get("t2mi_plp_id"), "i"
            ).lower()
            self.UseT2MI = True
...
class TransponderList(MenuList):
    ...
    def __init__(self):
        MenuList.__init__(
            self, [], enableWrapAround=True, content=eListboxPythonMultiContent
        )
        if FHD_Res:
            self.rowHight = 40
    ...
    def setEntries(self, transponderlist):
        transRolloff = {"0_35": "0.35", "0_25": "0.25", "0_20": "0.20", "Auto": "Auto"}
        transFec = {
            "FEC_AUTO": "Auto",
            "FEC_1_2": "1/2",
            "FEC_2_3": "2/3",
            ...
            if FHD_Res:
                tp.append(
                    MultiContentEntryText(
                        pos=(0, 0),
                        size=(105, self.rowHight),
                        font=0,
                        flags=RT_HALIGN_CENTER | RT_VALIGN_TOP,
                        text=transponder.system,
                        color=color,
                        border_width=1,
                        border_color=12092939,
                    )
                )
                tp.append(
                    MultiContentEntryText(
                        pos=(calc_xpos(tp), 0),
                        size=(95, self.rowHight),
                        font=0,
                        flags=RT_HALIGN_CENTER | RT_VALIGN_TOP,
                        text=str(int(float(transponder.frequency)) // 1000),
                        color=color,
                        border_width=1,
                        border_color=12092939,
                    )
                ...
                tp.append(
                    MultiContentEntryText(
                        pos=(calc_xpos(tp), 0),
                        size=(85, self.rowHight),
                        font=0,
                        flags=RT_HALIGN_CENTER | RT_VALIGN_TOP,
                        text=str(int(float(transponder.symbolrate)) // 1000),
                        color=color,
                        border_width=1,
                        border_color=12092939,
                    )
                ...
                tp.append(
                    MultiContentEntryText(
                        pos=(calc_xpos(tp), 0),
                        size=(75, self.rowHight),
                        font=0,
                        flags=RT_HALIGN_CENTER | RT_VALIGN_TOP,
                        text=transponder.isid,
                        color=color,
                        border_width=1,
                        border_color=12092939,
                    )
                ...
                tp.append(
                    MultiContentEntryText(
                        pos=(calc_xpos(tp), 0),
                        size=(110, self.rowHight),
                        font=0,
                        flags=RT_HALIGN_CENTER | RT_VALIGN_TOP,
                        text=transponder.plsmode,
                        color=color,
                        border_width=1,
                        border_color=12092939,
                    )
...
class TransponderEditor(Screen, ConfigListScreen, Transponder):
    ...
    def createConfig(self):
        self.configTransponderSystem = ConfigSelection(
            [("DVB-S", _("DVB-S")), ("DVB-S2", _("DVB-S2"))], self.system
        )
        self.configTransponderFrequency = ConfigFloat(
            default=[
                int(float(self.frequency)) // 1000,
                int(float(self.frequency)) % 1000,
            ],
        ...
        self.configTransponderSymbolrate = ConfigInteger(
            default=int(float(self.symbolrate)) // 1000, limits=(0, 99999)
        )
        ...
        self.configTransponderInversion = ConfigSelection(
            [("OFF", "off"), ("ON", "on"), ("AUTO", "auto")], self.inversion
        )
        ...
        self.configTransponderRollOff = ConfigSelection(
            [("0_35", "0.35"), ("0_25", "0.25"), ("0_20", "0.20"), ("Auto", "Auto")],
            self.rolloff,
        )
        self.configTransponderPilot = ConfigSelection(
            [("OFF", "off"), ("ON", "on"), ("AUTO", "auto")], self.pilot
        )
        self.configTransponderUseMultistream = ConfigYesNo(default=self.UseMultistream)
        self.configTransponderIsId = ConfigInteger(
            default=int(float(self.isid)), limits=(0, 255)
        )
        self.configTransponderPlsMode = ConfigSelection(
            [("Root", "Root"), ("Gold", "Gold"), ("Combo", "Combo")], self.plsmode
        )
        self.configTransponderPlsCode = ConfigInteger(
            default=int(float(self.plscode)), limits=(0, 262142)
        )
        ...
        self.configTransponderTsid = ConfigInteger(
            default=int(float(self.tsid)), limits=(0, 65535)
        )
        self.configTransponderOnid = ConfigInteger(
            default=int(float(self.onid)), limits=(0, 65535)
        )
        self.configTransponderUseT2MI = ConfigYesNo(default=self.UseT2MI)
        self.configTransponderT2miPlpId = ConfigInteger(
            default=int(float(self.t2mi_plp_id)), limits=(0, 255)
        )
        self.configTransponderT2miPid = ConfigInteger(
            default=int(float(self.t2mi_pid)), limits=(0, 8191)
        )
    ...
    def createSetup(self):
        ...
        if self.system == "DVB-S" or self.system == "DVB-S2":
            self.list.append(
                getConfigListEntry(_("Frequency"), self.configTransponderFrequency)
            )
            self.list.append(
                getConfigListEntry(
                    _("Polarization"), self.configTransponderPolarisation
                )
            )
            self.list.append(
                getConfigListEntry(_("Symbolrate"), self.configTransponderSymbolrate)
            )
        if self.system == "DVB-S":
        ...
        if self.system == "DVB-S" or self.system == "DVB-S2":
            self.list.append(
                getConfigListEntry(_("Inversion"), self.configTransponderInversion)
            )
        if self.system == "DVB-S2":
            self.list.append(
                getConfigListEntry(_("Modulation"), self.configTransponderModulation)
            )
            self.list.append(
                getConfigListEntry(_("RollOff"), self.configTransponderRollOff)
            )
            self.list.append(
                getConfigListEntry(_("Pilot"), self.configTransponderPilot)
            )
        if self.system == "DVB-S" or self.system == "DVB-S2":
            if self.system == "DVB-S2":
                self.list.append(
                    getConfigListEntry(
                        _("Use multistream"), self.configTransponderUseMultistream
                    )
                )
                if self.UseMultistream:
                    self.list.append(
                        getConfigListEntry(
                            _("Input Stream ID"), self.configTransponderIsId
                        )
                    )
                    self.list.append(
                        getConfigListEntry(_("PLS Mode"), self.configTransponderPlsMode)
                    )
                    self.list.append(
                        getConfigListEntry(_("PLS Code"), self.configTransponderPlsCode)
                    )
            if config.misc.tssateditorT2MI.value:
                if self.system == "DVB-S2":
                    self.list.append(
                        getConfigListEntry(
                            _("Use T2MI PLP"), self.configTransponderUseT2MI
                        )
                    )
                    if self.UseT2MI:
                        self.list.append(
                            getConfigListEntry(
                                "T2MI PLP ID", self.configTransponderT2miPlpId
                            )
                        )
                        self.list.append(
                            getConfigListEntry(
                                "T2MI PID", self.configTransponderT2miPid
                            )
                        )
            else:
                self.list.append(
                    getConfigListEntry(_("Use tsid"), self.configTransponderUseTsid)
                )
                if self.useTsid:
                    self.list.append(
                        getConfigListEntry(_("TSID"), self.configTransponderTsid)
                    )
                self.list.append(
                    getConfigListEntry(_("Use onid"), self.configTransponderUseOnid)
                )
                if self.useOnid:
                    self.list.append(
                        getConfigListEntry(_("ONID"), self.configTransponderOnid)
                    )
...
class TranspondersEditor(Screen):
    ...
    def layoutFinished(self):
        try:
            ...
            self["head"].l.setSelectionClip(
                eRect(data[1], data[0], data[3], data[4]), True
            )
    ...
    def removeTransponder(self):
        if len(self.transponderslist):
            ...
            self.session.openWithCallback(
                cb_func, MessageBox, _("Remove transponder?"), MessageBox.TYPE_YESNO
            )
...
class SatEditor(Screen, ConfigListScreen):
    ...
    def __init__(self, session, satelliteslist=None, satelliteData=None, clone=False):
        ...
        if self.satelliteData is not None:
            self.satelliteName = self.satelliteData.get(
                "name", "unknown satellite name"
            )
            if self.clone:
    ...
    def createConfig(self):
        self.configSatelliteName = ConfigText(
            default=self.satelliteName, visible_width=50, fixed_size=False
        )
        self.configSatelliteName.setUseableChars(
            "0,?!&@=*'+\()$~% 1.:;/-_#abc2ABCdef3DEFghi4GHIjkl5JKLmno6MNOpqrs7PQRStuv8TUVwxyz9WXYZ"
        )
        self.configSatellitePosition = ConfigFloat(
            default=self.satellitePosition, limits=[(0, 180), (0, 9)]
        )
        self.configSatelliteOrientation = ConfigSelection(
            [("east", "East"), ("west", "West")], self.satelliteOrientation
        )
    ...
    def createSetup(self):
        ...
        self.list.append(
            getConfigListEntry(_("Position"), self.configSatellitePosition)
        )
        self.list.append(
            getConfigListEntry(_("Orientation"), self.configSatelliteOrientation)
        )
        self.list.append(
            getConfigListEntry(_("Network scan"), self.configSatelliteFlagNetworkScan)
        )
    ...
    def okExit(self):
        ...
        satellitePosition = (
            self.configSatellitePosition.value[0] * 10
            + self.configSatellitePosition.value[1]
        )
        ...
        if satellitePosition == 0:
            self.session.open(
                MessageBox,
                _("Sorry, you can not use prime meridian."),
            ...
            for sat in self.satelliteslist:
                ...
                if satellitePosition == pos:
                    if exception:
                        if (
                            self.firstSatellitePosition != 0
                            and pos == self.firstSatellitePosition
                        ):
                            continue
                    self.session.open(
                        MessageBox,
                        _("This position number is busy.\nSelect another position."),
                        MessageBox.TYPE_ERROR,
                    )
                    return
        satelliteData = {
            "name": self.configSatelliteName.value,
...
class SatellitesEditor(Screen):
    ...
    def readSatellites(self, file, stop=True):
        ...
        try:
            ...
            satelliteslist = []
            for sat in satDict.iter("sat"):
                transponderslist = []
                for transponder in sat.iter("transponder"):
        ...
        try:
            ...
            for x in self.satelliteslist:
                ...
                for y in x[1]:
                    ...
                    transponder = xml.etree.cElementTree.SubElement(
                        satellite, "transponder", y
                    )
                    transponder.tail = "\n\t\t"
            ...
            os.rename(
                ...
                "/etc/enigma2/satellites.xml." + str(int(time())),
            )
    ...
    def addSatellite(self):
        ...
        def addAction(choice):
            if choice:
                ...
                if choice[1] == "new":
                    self.session.openWithCallback(
                        self.finishedSatAdd, SatEditor, self.satelliteslist
                    )
                elif choice[1] == "clone":
    ...
    def editTransponders(self):
        ...
        if cur_idx is None:
            return
        self.session.openWithCallback(
            self.finishedTranspondersEdit,
            TranspondersEditor,
            self.satelliteslist[cur_idx],
        )
    ...
    def compareColumn(self, a):
        ...
        if self.row[self.currentSelectedColumn][0] == "position":
            return int(float(a[0].get("position")))
    ...
    def blihdscanXML(self):
        ...
        menu = [
            (_("Show blindscan.xml"), "show"),
            (_("Add in user satellites.xml"), "add"),
        ]
        ...
        def addAction(choice):
            if choice:
                if choice[1] == "show":
                    self.session.open(
                        Console, _("Show blindscan.xml"), ["cat %s" % xml]
                    )
                elif choice[1] == "add":
